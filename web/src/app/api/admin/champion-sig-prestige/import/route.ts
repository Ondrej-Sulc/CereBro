...
} from "@cerebro/core/services/mcocSigPrestigeImportService"
...
export const POST = withRouteContext(async (req: NextRequest) => {
  try {
    ...
    const data = parseMcocSigPrestigeJson(text)
    const report = await importMcocSigPrestige(prisma, data, { write: true })
