...
import { Loader2, Upload, X, Bug, Clipboard, Maximize2, Hash, Award, Star, Zap, ZoomIn, ZoomOut, RotateCcw } from "lucide-react";
import { AnimatePresence, motion } from "framer-motion";
import Image from "next/image";
import axios from "axios";
import { Dialog, DialogContent, DialogTrigger, DialogTitle, DialogDescription } from "@/components/ui/dialog";
import { Badge } from "@/components/ui/badge";
import type { GridCell } from "@cerebro/core/services/roster/types";
import { UpdatedChampionItem, ChampionData } from "./UpdatedChampionItem";
import { TransformWrapper, TransformComponent, useControls } from "react-zoom-pan-pinch";

const ZoomControls = () => {
  const { zoomIn, zoomOut, resetTransform } = useControls();
  return (
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/50 backdrop-blur-sm p-1.5 rounded-full z-[100] pointer-events-auto">
      <button
        type="button"
        onClick={() => zoomOut()}
...
interface DebugGridCell extends GridCell {
            ...
            <Card key={i} className="border-slate-800 bg-slate-900/50 overflow-hidden">
              ...
              <CardContent className="space-y-6">
                <Dialog>
                  ...
                  <DialogContent className="max-w-[95vw] w-[1400px] max-h-[95vh] h-[900px] p-0 bg-slate-950 border-slate-800 flex items-center justify-center">
                      ...
                      >
                        {() => (
                          <>
                            <ZoomControls />
                            <TransformComponent
                              wrapperClass="!w-full !h-full flex items-center justify-center bg-slate-950"
                        ...
                        return (
                          <UpdatedChampionItem
                            ...
                            item={{
                              ...
                              stars: cell.stars || 0,
                              rank: cell.rank || 0,
                              isAwakened: typeof cell.sigLevel === 'number',
